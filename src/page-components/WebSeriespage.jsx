...
const getGenreDisplayName = (genreName) => {
  ...
  // If it's already in Hindi or has a mapping
...
export default function WebSeriesPage({ serverCategory, initialSeries = [], initialGenres = [] }) {
  ...
  const availableYears = useMemo(() =>
    [...new Set(series.map(s => String(s.year || (s.releaseDate ? new Date(s.releaseDate).getFullYear() : ""))).filter(Boolean))].sort((a, b) => b - a),
    [series]
  );
  ...
  const availableRatings = useMemo(() =>
    [...new Set(series.map(s => {
      const r = s.rating;
      if (!r) return null;
      return typeof r === 'object' ? String(r.title || r.name || r.value || "") : String(r);
  ...
  const availableCertificates = useMemo(() =>
    [...new Set(series.map(s => {
      const c = s.age_rating || s.certificate;
      if (!c) return null;
  ...
  const availableLanguages = useMemo(() =>
    [...new Set(series.flatMap(s => s.languages?.map(l => l.language || l.name) || []).filter(Boolean))].sort(),
    [series]
  );

  const availablePlatforms = useMemo(() =>
    [...new Set(series.flatMap(s => s.watchingPlatform?.map(p => p.platform) || []).filter(Boolean))].sort(),
    [series]
  );

  const applyFilter = (key, value) => setFilters(prev => ({ ...prev, [key]: value }));
  const clearAllFilters = () => setFilters({ genre: "All", year: "All", rating: "All", certificate: "All", language: "All", platform: "All" });
    ...
    return series.filter((item) => {
      if (filters.genre !== "All") {
        const hasGenre = item.genres?.some(g => {
          const genreName = typeof g === 'object' ? g.name : g;
          return genreName === filters.genre;
        });
        if (!hasGenre) return false;
      }
      if (filters.rating !== "All") {
        const itemRating = typeof item.rating === 'object' ? String(item.rating.value || item.rating.title) : String(item.rating);
        if (itemRating !== filters.rating) return false;
      }
      if (filters.certificate !== "All") {
        const itemCert = typeof (item.age_rating || item.certificate) === 'object' ? String(item.age_rating?.name || item.certificate?.title) : String(item.age_rating || item.certificate);
        if (itemCert !== filters.certificate) return false;
      }
      if (filters.year !== "All") {
        const itemYear = String(item.year || (item.releaseDate ? new Date(item.releaseDate).getFullYear() : ""));
      ...
      if (filters.platform !== "All") {
        const hasPlatform = item.watchingPlatform?.some(p => p.platform === filters.platform);
  ...
  const showAllSeriesWithGenres = Object.values(filters).every(v => v === "All");
 ...
 const PLATFORM_ICONS = {
  ...
  const WebSeriesCard = ({ series: item }) => {
    const posterUrl = typeof item.poster === 'string' ? item.poster : item.poster?.url || null;
    const year = item.year || (item.releaseDate ? new Date(item.releaseDate).getFullYear() : "N/A");
    
    const platforms = Array.isArray(item.watchingPlatform) 
      ? item.watchingPlatform.slice(0, 3) 
      : [];
    const hasMorePlatforms = Array.isArray(item.watchingPlatform) && item.watchingPlatform.length > 3;

    return (
      <div className="group block">
        <Link href={`/${serverCategory}/web-series/${item.slug}`}>
          <div className="relative aspect-[2/3] rounded-lg overflow-hidden mb-2 bg-gray-200 dark:bg-gray-900">
            ...
            {item.rating && (
              <div className="absolute top-2 right-2 bg-black/80 text-yellow-400 px-2 py-1 rounded text-sm font-bold">
                ⭐ {typeof item.rating === 'object' ? (item.rating.title || item.rating.name) : item.rating}
              </div>
            )}
          </div>
        ...
        {platforms.length > 0 && (
          <div className="mt-2 flex flex-wrap items-center gap-1.5">
            {platforms.map((platform, idx) => {
              const platformName = platform.platform?.replace(/^\s+|\s+$/g, '') || 'प्लेटफॉर्म';
            ...
            {hasMorePlatforms && (
              ...
              >
                <span>+{item.watchingPlatform.length - 3}</span>
                <ChevronRight size={8} />
              </Link>
            )}
          </div>
        ...
        {platforms.length === 0 && item.watchingPlatform?.[0]?.watch_status && (
          <div className="mt-2">
            <span className="inline-block px-2 py-1 text-[10px] font-medium bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-500 rounded border border-gray-200 dark:border-gray-700">
              {item.watchingPlatform[0].watch_status === 'subscription' ? 'जल्द आ रहा है' : 'स्ट्रीमिंग के लिए उपलब्ध'}
            </span>
          </div>
        )}
      </div>
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 bg-[#f6f6f6] p-6 rounded-2xl dark:bg-gray-800">
        ...
        <div className="border-b border-gray-300 dark:border-gray-700 py-4 mb-4 flex flex-row gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-300 mt-1">
              विभिन्न जीनर, रेटिंग और रिलीज वर्षों में वेब सीरीज का विशाल संग्रह देखें
            </p>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${filters.language === "All"
            ...
            {availableLanguages.map(l => (
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${filters.language === l
                  ? "bg-pink-600 text-white shadow-lg shadow-pink-600/30"
                  : "bg-gray-200 dark:bg-neutral-800 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-neutral-700"
                  }`}
              >
        ...
        <div className="py-6">
          {loading ? (
          ...
          ) : showAllSeriesWithGenres ? (
            <div className="space-y-12">
              {Object.entries(seriesByGenre).length === 0 && filteredItems.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
                  {filteredItems.map(item => (
                    <WebSeriesCard key={item.id} series={item} />
                  ))}
                </div>
              ) : Object.entries(seriesByGenre).length === 0 ? (
                <div className="text-center py-20">
                  <div className="text-6xl mb-4">📺</div>
                  <p className="text-gray-600 dark:text-gray-400 text-lg mb-2">कोई वेब सीरीज उपलब्ध नहीं है</p>
                  <p className="text-gray-500 dark:text-gray-500 text-sm">कृपया बाद में देखें</p>
                </div>
              ) : (
                Object.entries(seriesByGenre)
                  .sort(([, a], [, b]) => b.length - a.length)
                  .map(([genreName, genreSeries]) => {
                    const isExpanded = expandedGenres[genreName];
                    const displaySeries = isExpanded ? genreSeries : genreSeries.slice(0, 6);
                    ...
                    return (
                      <div key={genreName}>
                        <div className="flex items-center justify-between mb-6">
                          <h2 className="text-gray-900 dark:text-white text-2xl font-bold">
                            ...
                            <span className="text-gray-500 dark:text-gray-400 text-lg ml-2 font-normal">
                              ({genreSeries.length} सीरीज)
                            </span>
                          </h2>

                          {genreSeries.length > 6 && (
                            <button
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 card-theme">
            ...
            <>
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-gray-900 dark:text-white text-2xl font-bold">
                  {filters.genre !== "All" ? getGenreDisplayName(filters.genre) : "वेब सीरीज"}
                  <span className="text-gray-500 dark:text-gray-400 text-lg ml-2 font-normal">
                    ({filteredItems.length} सीरीज)
                  </span>
                </h2>
              </div>
              ...
              {filteredItems.length > 0 ? (
                ...
                <div className="text-center py-20">
                  <div className="text-6xl mb-4">📺</div>
                  <p className="text-gray-600 dark:text-gray-400 text-lg mb-2">कोई वेब सीरीज नहीं मिली</p>
